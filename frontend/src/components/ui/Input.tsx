  ...
  return (
    <div className="w-full">
      {label && (
        <label
          htmlFor={id}
          className="block text-[11px] font-black text-[#6B6B6B] mb-2 tracking-[0.2em] ml-1 uppercase"
        >
          {label}
        </label>
      )}
      <input
        id={id}
        className={[
          'w-full h-11 px-4 text-[15px] font-bold text-[#1E1E1E] placeholder-[#A0A0A0]',
      ...
      {helperText && !error && <p className="mt-1.5 text-[12px] text-[#6B6B6B] font-bold">{helperText}</p>}
    </div>
  );
};
